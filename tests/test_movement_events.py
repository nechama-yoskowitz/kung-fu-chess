...
def test_rook_4_cells_produces_4_events():
    move = make_move("wR", 0, 0, 0, 4, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 4
    assert events[0] == MovementEvent(0, "wR", 0, 1, 1000, 0, False)
    assert events[1] == MovementEvent(0, "wR", 0, 2, 2000, 1, False)
    assert events[2] == MovementEvent(0, "wR", 0, 3, 3000, 2, False)
    assert events[3] == MovementEvent(0, "wR", 0, 4, 4000, 3, True)
...
def test_bishop_3_cells_diagonal():
    move = make_move("wB", 0, 0, 3, 3, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 3
...
def test_queen_horizontal_5_cells():
    move = make_move("wQ", 3, 0, 3, 5, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 5
...
def test_king_one_step():
    move = make_move("wK", 4, 4, 3, 5, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 1
    assert events[0] == MovementEvent(0, "wK", 3, 5, 1000, 0, True)
...
def test_pawn_one_step():
    move = make_move("wP", 6, 0, 5, 0, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 1
    assert events[0] == MovementEvent(0, "wP", 5, 0, 1000, 0, True)
...
def test_pawn_two_step():
    move = make_move("wP", 6, 0, 4, 0, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 2
    assert events[0] == MovementEvent(0, "wP", 5, 0, 1000, 0, False)
    assert events[1] == MovementEvent(0, "wP", 4, 0, 2000, 1, True)
...
def test_knight_single_event_at_arrive_at():
    ...
    move = make_move("wN", 4, 4, 2, 5, started_at=0)
    events = decompose_moves_to_events([move])
    assert len(events) == 1
...
def test_two_moves_different_times_sorted_chronologically():
    ...
    rook = PendingMove("wR", 0, 0, 0, 2, started_at=0, arrive_at=2000, sequence_id=0)
    king = PendingMove("wK", 1, 1, 1, 2, started_at=500, arrive_at=1500, sequence_id=1)
    ...
    assert events[0].piece == "wR"
    assert events[1].event_time == 1500
    assert events[1].piece == "wK"
    assert events[2].event_time == 2000
    assert events[2].piece == "wR"
...
def test_same_time_tiebreak_by_creation_order():
    ...
    move_a = PendingMove("wR", 0, 0, 0, 1, started_at=0, arrive_at=1000, sequence_id=0)
    move_b = PendingMove("wB", 1, 1, 0, 0, started_at=0, arrive_at=1000, sequence_id=1)
...
def test_last_event_matches_arrive_at():
    move = make_move("wR", 0, 0, 0, 3, started_at=500)
    events = decompose_moves_to_events([move])
    last = events[-1]
...
def test_knight_last_event_matches_arrive_at():
    move = make_move("wN", 0, 0, 2, 1, started_at=200)
    events = decompose_moves_to_events([move])
...
def test_started_at_offset_applied_correctly():
    ...
    move = PendingMove("wR", 0, 0, 0, 3, started_at=5000, arrive_at=8000, sequence_id=0)
    events = decompose_moves_to_events([move])
